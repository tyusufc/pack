...
class CMouseController(ui.NoWindow):
	...
	def AttachMoney(self, owner, type, count, cheque=0):
		...
		self.AttachedIconHalfHeight = grpImage.GetHeight(self.AttachedIconHandle) / 2
		if app.ENABLE_CHEQUE_SYSTEM:
			self.AttachedCheque = cheque
			if not count and cheque > 0:
		...
		if count > 1 and self.countNumberLine:
			self.countNumberLine.SetNumber(str(count))
			self.countNumberLine.Show()

	if app.ENABLE_CHEQUE_SYSTEM:
		def GetAttachedChequeAmount(self):
			if True == self.isAttached():
				if player.ITEM_MONEY == self.GetAttachedItemIndex():
					return self.GetAttachedItemCheque()
			return 0

		def GetAttachedChequeCount(self):
			return self.GetAttachedChequeAmount()

		def GetAttachedItemCheque(self):
			if False == self.isAttached():
				return 0
			return self.AttachedCheque
