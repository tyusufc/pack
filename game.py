...
#
	...
	def OnKeyDown(self, key):
		if key == app.DIK_K:
			chat.AppendChat(chat.CHAT_TYPE_INFO, "K tuşuna basıldı!")
			self.ToggleAutoHuntWindow()
		return True

	def ToggleAutoHuntWindow(self):
		if not self.autoHuntWindow:
			self.autoHuntWindow = uiAutoHunt.AutoHunt()
			self.autoHuntWindow.Show()
		else:
			if self.autoHuntWindow.IsShow():
				self.autoHuntWindow.Hide()
			else:
				self.autoHuntWindow.Show()
