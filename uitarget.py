...
class TargetBoard(ui.ThinBoard):
	...
	if app.ENABLE_TARGET_INFORMATION_SYSTEM:
		class InfoBoard(ui.ThinBoard):
			...
			def __init__(self):
				...
				wndItem.ArrangeSlot(0, 12, self.slotSize/12, 32, 32, 0, 0)
				wndItem.RefreshSlot()
			...
			def Refresh(self):
				...
				tempHeight = ((self.slotSize / 12)-4)*32
			...
			def __LoadInformation(self, race):
				...
				self.wndItem.ArrangeSlot(0, 12, self.slotSize/12, 32, 32, 0, 0)
				...
				for value in xrange(self.slotSize):
					...
					if 0 == self.itemArray[value]:	# ?????
						...
						for i in range(size - 1):
							emptySlotIndex = emptySlotIndex + 12
					...
					if len(constInfo.MONSTER_INFO_DATA[race]["items"]) == 0:
						...
						for curItem in constInfo.MONSTER_INFO_DATA[race]["items"]:
							...
							if -1 != emptySlotPos:
								...
								if itemSize[1] == 2:
									self.itemArray[emptySlotPos + 12] = 1
								elif itemSize[1] == 3:
									self.itemArray[emptySlotPos + 12] = 1
									self.itemArray[emptySlotPos + 24] = 1
			...
			def AppendSeperator(self):
				img = ui.ImageBox()
				img.LoadImage("d:/ymir work/ui/target_info/seperator.tga")
	...
	def __init__(self):
		...
		if app.ENABLE_TARGET_INFORMATION_SYSTEM:
			...
			infoButton.SetUpVisual("d:/ymir work/ui/target_info/question_mark_1.tga")
			infoButton.SetOverVisual("d:/ymir work/ui/target_info/question_mark_2.tga")
			infoButton.SetDownVisual("d:/ymir work/ui/target_info/question_mark_3.tga")
